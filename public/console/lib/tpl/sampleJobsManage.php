                            ...
                            foreach ($job['tubes'] as $tubename => $val) {
                                if (isset($_server) && !empty($_server)) {
                                    ...
                                    <a class="btn btn-default  btn-sm"
                                       href="./index.php?server=<?php echo $_server ?>&tube=<?php echo urlencode($tubename) ?>&action=loadSample&key=<?php echo $key; ?>&redirect=<?php echo urlencode('./?action=manageSamples'); ?>"><i
                                        ...
                                        <ul class="dropdown-menu">
                                            ...
                                            if (is_array($_servers)) {
                                                foreach ($_servers as $server2) {
                                                    ?>
                                                    <li>
                                                        <a href="./index.php?server=<?php echo $server2 ?>&tube=<?php echo urlencode($tubename) ?>&action=loadSample&key=<?php echo $key; ?>&redirect=<?php echo urlencode('./?action=manageSamples'); ?>"><?php echo $server2; ?></a>
                                                    </li>
                                                <?php
                                                }
                                            }
