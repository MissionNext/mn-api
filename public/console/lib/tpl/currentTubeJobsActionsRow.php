...
<section id="actionsRow">
    <b>Actions:</b>&nbsp;
    <a class="btn btn-default btn-sm" href="./index.php?server=<?php echo $server ?>&tube=<?php echo urlencode($tube) ?>&action=kick&count=1"><i class="glyphicon glyphicon-forward"></i> Kick 1 job</a>
    <a class="btn btn-default btn-sm" href="./index.php?server=<?php echo $server ?>&tube=<?php echo urlencode($tube) ?>&action=kick&count=10"
       ...
       if (empty($tubeStats['pause-time-left'])) {
           ?><a class="btn btn-default btn-sm" href="./index.php?server=<?php echo $server ?>&tube=<?php echo urlencode($tube) ?>&action=pause&count=-1"
        ...
        ?><a class="btn btn-default btn-sm" href="./index.php?server=<?php echo $server ?>&tube=<?php echo urlencode($tube) ?>&action=pause&count=0"
        ...
        <ul class="dropdown-menu">
            ...
            if (is_array($sampleJobs) && count($sampleJobs)) {
                foreach ($sampleJobs as $key => $name) {
                    ?>
                    <li>
                        <a href="./index.php?server=<?php echo $server ?>&tube=<?php echo urlencode($tube) ?>&action=loadSample&key=<?php echo urlencode($key); ?>"><?php echo htmlspecialchars($name); ?></a>
                    </li>
                    <?php
                }
                ?>
