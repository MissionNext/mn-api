...
<section id="actionsRow">
    <a class="btn btn-default btn-sm" href="./index.php?server=<?php echo $server ?>&tube=<?php echo urlencode($tube) ?>"><i class="glyphicon glyphicon-backward"></i>  &nbsp;Back to tube</a>
</section>
<?php
                                        ...
                                        <div class="dropdown btn-group-xs">
                                            ...
                                            <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1">
                                                <li role="presentation"><a role="menuitem" class="addSample" data-jobid="<?php echo $job->getId(); ?>"
                                                                           href="./index.php?server=<?php echo $server ?>&tube=<?php echo urlencode($tube) ?>&action=addSample">
                                                        <i class="glyphicon glyphicon-plus glyphicon-white"></i>
                                                        Add to samples</a>
                                                </li>
                                                <li role="presentation"><a role="menuitem"
                                                                           href="./index.php?server=<?php echo $server ?>&tube=<?php echo urlencode($tube) ?>&state=<?php echo $state ?>&action=deleteJob&jobid=<?php echo $job->getId(); ?>"><i
                                                            class="glyphicon glyphicon-remove glyphicon-white"></i>
                                                        Delete</a>
                                                </li>
                                                <li role="presentation"><a role="menuitem"
                                                                           href="./index.php?server=<?php echo $server ?>&tube=<?php echo urlencode($tube) ?>&state=<?php echo $state ?>&action=kickJob&jobid=<?php echo $job->getId(); ?>"><i
